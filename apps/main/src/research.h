...
public:
    ...
    using materials_t = std::vector<std::pair<type_t,size_t>>;
    using skills_t = std::vector<skill_requirement_t>;
    using duration_t = std::chrono::seconds;
    using baseProbability_t = double;

    Research(blueprint_t blueprint_, product_t product_, size_t producedQuantity_, materials_t materials_, skills_t skills_, duration_t duration_, baseProbability_t baseProbability_) : blueprint(blueprint_), product(product_), producedQuantity(producedQuantity_), materials(std::move(materials_)), skills(std::move(skills_)), duration(duration_), baseProbability(baseProbability_) {}

    blueprint_t getBlueprint() const { return blueprint; }
    product_t getProduct() const { return product; }
    size_t getProducedQuantity() const { return producedQuantity; }
