...
public:
    ...
    Research(blueprint_t blueprint_, product_t product_, std::size_t producedQuantity_, materials_t materials_, skills_t skills_, duration_t duration_, baseProbability_t baseProbability_) : blueprint(blueprint_), product(product_), producedQuantity(producedQuantity_), materials(std::move(materials_)), skills(std::move(skills_)), duration(duration_), baseProbability(baseProbability_) {}
