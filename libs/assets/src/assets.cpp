...
void NormalAssets::loadAssetsFromXml(const Names& names) {
    assets_.reserve(1000);
    ...
    for (const auto& asset : assetlist) {
        ...
        auto assetIt = assets_.insert({typeId, 0}); // no-op if it already exists.
        assetIt.first->second += quantity;
    }
}

void NormalAssets::addAssetsFromTsv(const char* filename, const Names& names) {
    std::cout << "Assets (" << filename << "):\n";
...
void NormalAssets::loadAssetsFromTsv(const Names& names) {
    assets_.reserve(1000);
    addAssetsFromTsv("materials.txt", names);
    addAssetsFromTsv("products.txt", names);
}

void OwnedBPCs::loadOwnedBPCsFromXml(const Names& names) {
    ...
    const auto& owned_blueprints =
        doc.child("eveapi")
            ...
            .find_child([](const pugi::xml_node& n) { return n.attribute("name").as_string() == assetsString; })
            .children();
    ownedBPCs_.reserve(std::size_t(std::distance(owned_blueprints.begin(), owned_blueprints.end())));
    for (const auto& bp : owned_blueprints) {
        ...
        auto bpIt = ownedBPCs_.insert({x, 0}); // no-op if it already exists.
...
std::size_t NormalAssets::countAsset(std::size_t typeId) const {
    auto a = assets_.find(typeId);
    return a != assets_.end() ? a->second : 0;
}

std::size_t PlanetaryAssets::countPlanetaryAsset(std::size_t typeId) const {
    auto a = planetary_assets_.find(typeId);
    return a != planetary_assets_.end() ? a->second : 0;
}

std::size_t OwnedBPCs::runs(const BlueprintWithEfficiency& bpwe) const {
    auto r = ownedBPCs_.find(bpwe);
    return r != ownedBPCs_.end() ? r->second : 0;
}

std::size_t OwnedBPCs::runs(std::size_t typeId) const {
    auto runs = 0ul;
    for (const auto& ownedBP : ownedBPCs_)
