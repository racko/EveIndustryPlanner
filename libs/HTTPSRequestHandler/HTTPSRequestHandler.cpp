...
static Response::Status convert(Poco::Net::HTTPResponse::HTTPStatus status) {
    return static_cast<Response::Status>(static_cast<int>(status));
}
...
static void handleRequestNoRedirect(Poco::Net::HTTPClientSession& client,
                                    const Request& request,
                                    const std::string& data,
                                    NameValueCollection& cookies,
                                    std::function<void(const Response& response, std::istream& s)> f) {
    const auto& method = request.getMethod() == Request::Method::GET ? Poco::Net::HTTPRequest::HTTP_GET
...
static void handleRedirect(Poco::Net::HTTPClientSession& client,
                           NameValueCollection& cookies,
                           std::function<void(const Response& response, std::istream& s)> f,
    ...
    if (status == Response::Status::HTTP_FOUND || status == Response::Status::HTTP_MOVED_PERMANENTLY) {
        ...
        handleRequestNoRedirect(
            client, Request(response["Location"]), "", cookies, [&](const auto& response_, auto& s_) {
                handleRedirect(client, cookies, f, response_, s_);
            });
...
static void handleRequest(Poco::Net::HTTPClientSession& client,
                          const Request& request,
                          const std::string& data,
                          NameValueCollection& cookies,
                          std::function<void(const Response& response, std::istream& s)> f) {
    if (request.getFollowRedirects() == Request::OnRedirect::FOLLOW) {
        handleRequestNoRedirect(client, request, data, cookies, [&](const auto& response, auto& s) {
            handleRedirect(client, cookies, f, response, s);
        });
    } else {
...
struct HTTPSRequestHandler::impl {
    impl(std::string host, std::uint16_t port)
        : session(std::move(host), port, context), work(ioService), thread([&] { ioService.run(); }) {
...
HTTPSRequestHandler::HTTPSRequestHandler(std::string host, std::uint16_t port)
    : pimpl(new impl(std::move(host), port)) {}

HTTPSRequestHandler::~HTTPSRequestHandler() = default;

void HTTPSRequestHandler::postRequest(const std::shared_ptr<RequestWithResponseHandler>& request,
                                      const std::string& data) {
    pimpl->ioService.post([&, request, data] {
        try {
            ::handleRequest(pimpl->session, *request, data, pimpl->cookies, [request](const auto& response, auto& s) {
                if (response.getStatus() >= Response::Status::HTTP_OK &&
                    response.getStatus() < Response::Status::HTTP_MULTIPLE_CHOICES)
...
void print(const Response&, std::istream& s) { std::cout << s.rdbuf(); }

void ignore(const Response&, std::istream& s) { s.ignore(std::numeric_limits<std::streamsize>::max()); }
...
HTTPSRequestHandlerGroup::HTTPSRequestHandlerGroup(std::size_t n, const std::string& host, std::uint16_t port)
    : pimpl(std::make_unique<impl>(n, host, port)) {}
HTTPSRequestHandlerGroup::~HTTPSRequestHandlerGroup() = default;

void HTTPSRequestHandlerGroup::postRequest(const std::shared_ptr<RequestWithResponseHandler>& request,
                                           const std::string& data) {
    pimpl->postRequest(request, data);
}

void HTTPSRequestHandlerGroup::postRequest(RequestWithResponseHandler& request, const std::string& data) {
    pimpl->postRequest(request, data);
}
