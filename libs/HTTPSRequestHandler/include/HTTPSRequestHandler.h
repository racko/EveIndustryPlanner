...
public:
    ...
    Request(Method method, std::string location, OnRedirect follow_redirects = OnRedirect::FOLLOW) : method_(method), location_(std::move(location)), follow_redirects_(follow_redirects) {}
...
public:
    ...
    Response(Status status, std::string reason, Iterator start, const Iterator stop) : status_(status), reason_(reason), header_(start, stop) {}
...
class ResponseIgnoringRequest : public RequestWithResponseHandler {
    ...
    void handleResponse(const Response& response, std::istream& s) override {
        ignore(response, s);
    }
...
public:
    RequestWithPolicy(Method method, std::string location, Functor f, OnRedirect follow_redirects = OnRedirect::FOLLOW) : RequestWithResponseHandler(method, std::move(location), follow_redirects), f_(f) {}
...
public:
    HTTPSRequestHandler(std::string host, uint16_t port);
