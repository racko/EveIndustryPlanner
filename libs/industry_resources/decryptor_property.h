...
class DecryptorProperty {
  public:
    DecryptorProperty(boost::optional<TypeID> d, BlueprintEfficiency be, unsigned r, double p)
        : decryptor(std::move(d)), blueprintEfficiency(std::move(be)), runModifier(r), probabilityModifier(p) {}
    DecryptorProperty(boost::optional<TypeID> d, double me, double te, unsigned r, double p)
        : decryptor(std::move(d)), blueprintEfficiency(me, te), runModifier(r), probabilityModifier(p) {}
